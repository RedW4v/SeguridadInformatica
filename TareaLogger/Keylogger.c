...
char get_char_from_keycode(int keycode) {
    switch (keycode) {
        ...
        case KEY_EQUAL: return shift_pressed ? '0':'0';
        ...
        case KEY_CAPSLOCK: return 0; // No visible character
            return 0; // Function keys
        default: return 0; // No mapping for this keycode
    }
}
    ...
    while (1) {
        ...
        if (ev.type == EV_KEY) {
            if (ev.value == 1) { // Key press event
                ...
                char c = get_char_from_keycode(ev.code);
                if (c) {
                    fputc(c, fp);
